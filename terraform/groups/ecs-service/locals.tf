...
locals {
  stack_name                 = "payments" # this must match the stack name the service deploys into
  name_prefix                = "${local.stack_name}-${var.environment}"
  global_prefix              = "global-${var.environment}"
  service_name               = "refund-request-consumer"
  container_port             = "8080"
  docker_repo                = "refund-request-consumer"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  healthcheck_path           = "/refund-request-consumer/healthcheck"
  healthcheck_matcher        = "200"
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "refund-request-consumer.env"
